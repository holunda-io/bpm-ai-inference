...
async def test_translate():
    ...
    model = EasyNMT()
    translated = await model.translate(given_text, target_language)

    assert translated == expected_translated


async def test_translate_same():
    given_text = "Das ist ein Test"
    target_language = "de"
...
async def test_translate_multiple():
    given_texts = ["Das ist ein Test", "Un altro test"]
    target_language = "en"
    expected_translated = ["This is a test", "Another test"]

    model = EasyNMT()
    translated = await model.translate(given_texts, target_language)
